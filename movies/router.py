...
async def get_movie(
    ...
    """Get complete movie data: metadata + downloads + streams"""
    ...
    # 4. Get vidsrc streams if requested and imdb_id available
    streams = []
    ...
    # 6. Extract poster with dimensions
    poster = None
    if movie_item and movie_item.cover:
        poster = {
            "url": str(movie_item.cover.url),
            "width": movie_item.cover.width,
            "height": movie_item.cover.height,
            "size_kb": round(movie_item.cover.size / 1024, 2) if movie_item.cover.size else 0
        }
    
    # 7. Extract backdrop/stills
    backdrop = None
    ...
    if omdb_data:
        for rating in omdb_data.get("Ratings", []):
            if rating.get("Source") == "Rotten Tomatoes":
                response["data"]["movie"]["ratings"]["rotten_tomatoes"] = rating.get("Value")
            elif rating.get("Source") == "Metacritic":
                response["data"]["movie"]["ratings"]["metacritic"] = rating.get("Value")
    
    return response

@router.get("/{title}/downloads")
async def get_movie_downloads(title: str, year: Optional[int] = None):
    """Get only download URLs for a movie"""
