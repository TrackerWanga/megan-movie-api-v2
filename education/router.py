...
@router.get("/search")
async def search_education(q: str = Query(...), limit: int = Query(20)):
    search = Search(session, query=q, subject_type=SubjectType.EDUCATION)
    try:
        results = await search.get_content_model()
        items = [format_education_item(item) for item in results.items[:limit]]
...
async def get_education_details(detail_path: str):
    search = Search(session, query=detail_path, subject_type=SubjectType.EDUCATION)
    results = await search.get_content_model()
    ...
    subject_id = str(edu_item.subjectId)
    
    # Get qualities
    qualities = []
    try:
        dl_obj = DownloadableSingleFilesDetail(session, edu_item)
